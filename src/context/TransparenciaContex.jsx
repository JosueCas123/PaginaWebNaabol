...
export const TransparenciaContex = createContext()
...
export const TransparenciaProvider = ({children}) => {
  ...
  useEffect(() => {
      const obtenerNoticias = async() => {
        ...
        try {
          ...
          const url = 'http://127.0.0.1:8000/api/noticias_area';
  ...
  const eliminarNoticia =async ({id})=> {
      ...
      if(confirmar){
          const urlNoticia = `http://127.0.0.1:8000/api/noticia_delete/${id}`
          const token = localStorage.getItem('token');
          try {
  ...
  return (
    <TransparenciaContex.Provider value={{
      noticias,
            setNoticias,
            guardarNoticia,
            eliminarNoticia,
